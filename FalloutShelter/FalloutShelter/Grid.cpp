...
void Grid::InitTiles()
{
	...
	for (int _y = 0; _y < tilesCount.x; _y++)
	{
		...
		for (int _x = 0; _x < tilesCount.y; _x++)
		{
			if (_x==0 && _y ==0)
			{
				Tile* tile = new Tile(Vector2f(_x * WINDOW_SIZE.x / tilesCount.x, _y * WINDOW_SIZE.y / tilesCount.y), ET_EXTERNE, _tileSize);
				_tiles.push_back(tile);
				continue;
			}
			Tile* tile = new Tile(Vector2f(_x* WINDOW_SIZE.x / tilesCount.x, _y * WINDOW_SIZE.y / tilesCount.y),ET_BUILDABLE, _tileSize);
			_tiles.push_back(tile);
		}
